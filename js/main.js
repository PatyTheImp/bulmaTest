...
async function populateDropdown() {
    ...
    try {
        ...
        data.forEach(d => {
            ...
            filmsSelect.insertAdjacentHTML('beforeend', `<option value="${d.id}">${d.title}</option>`);
        });
    }
    catch (error) {
...
function renderMovieInfo(data) {
    ...
    const html = `
        <h1 class="title is-2 film-title">${data.title}</h1>
        <h1 class="title original-title">${data.original_title}</h1>
        <p class="subtitle">${data.original_title_romanised}</p>

        <p class="has-text-justified"><b>Description: </b>${data.description}</p>
